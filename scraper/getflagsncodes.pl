...
for my $tr ( $tree->findnodes('//*/tr') ) {
    my @content = $tr->content_list;

    my $code = $content[1]->as_text();
    next unless $code =~ /[A-Z]{3}/;

    p $code;

    my @flaglist = $content[0]->content_list;
    my @iconlist = $flaglist[0]->content_list;
    my $iconuri  = $iconlist[0]->{src};

    # p $iconuri;


    my $name = $flaglist[1]->as_text;
    p $name;
    ...
    for my $size ( 16, 24, 32, 48, 64 ) {
        my $rps   = $size . "px";
        my $rsuri = $iconuri =~ s/\d\dpx/$rps/r;    #/
...
open my $fh, '>', '../flags.json';

say $fh $json->encode($flagcodes);
close $fh; 
